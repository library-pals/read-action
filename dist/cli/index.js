...
var mapObj = __nccwpck_require__(2126);
var decamelize = __nccwpck_require__(208);
...
const isPlainObject = __nccwpck_require__(864);
const arrify = __nccwpck_require__(7129);
...
const gitHosts = __nccwpck_require__(9913)
...
const url = __nccwpck_require__(7310)
const gitHosts = __nccwpck_require__(9913)
const GitHost = module.exports = __nccwpck_require__(9718)
const LRU = __nccwpck_require__(4722)
const cache = new LRU({ max: 1000 })
...
var url = __nccwpck_require__(7310)
var typos = __nccwpck_require__(52)
...
var util = __nccwpck_require__(3837)
var messages = __nccwpck_require__(5632)
...
// EXTERNAL MODULE: ./node_modules/minimist-options/index.js
var minimist_options = __nccwpck_require__(9658);
...
function camelcase_camelCase(input, options) {
	...
	const makeMapper = parentPath => (key, value) => {
		...
		if (!(exclude && has(exclude, key))) {
			...
			if (cache.has(cacheKey)) {
				...
				const returnValue = camelcase_camelCase(key, {pascalCase, locale: false});
...
function decamelize_decamelize(
	text,
	{
		separator = '_',
		preserveConsecutiveUppercase = false,
...
// EXTERNAL MODULE: ./node_modules/decamelize-keys/index.js
var decamelize_keys = __nccwpck_require__(4598);
;// CONCATENATED MODULE: ./node_modules/trim-newlines/index.js
...
var min_indent = __nccwpck_require__(8236);
...
;// CONCATENATED MODULE: ./node_modules/redent/index.js
...
// EXTERNAL MODULE: ./node_modules/read-pkg/node_modules/normalize-package-data/lib/normalize.js
var lib_normalize = __nccwpck_require__(5358);
...
async function read_pkg_readPackage({cwd, normalize = true} = {}) {
...
// EXTERNAL MODULE: ./node_modules/hard-rejection/index.js
var hard_rejection = __nccwpck_require__(5597);
// EXTERNAL MODULE: ./node_modules/normalize-package-data/lib/normalize.js
var normalize = __nccwpck_require__(3188);
;// CONCATENATED MODULE: ./node_modules/meow/index.js
...
const reportMissingRequiredFlags = missingRequiredFlags => {
	...
	for (const flag of missingRequiredFlags) {
		console.error(`\t--${decamelize_decamelize(flag.key, {separator: '-'})}${flag.alias ? `, -${flag.alias}` : ''}`);
	}
};
...
const meow = (helpText, options = {}) => {
	...
	const foundPackage = readPackageUpSync({
		cwd: (0,external_node_path_namespaceObject.dirname)((0,external_node_url_namespaceObject.fileURLToPath)(options.importMeta.url)),
		normalize: false,
	});

	options = {
		pkg: foundPackage ? foundPackage.packageJson : {},
		argv: external_node_process_namespaceObject.argv.slice(2),
		flags: {},
		inferType: false,
		input: 'string',
		help: helpText,
	...
	parserOptions = decamelize_keys(parserOptions, '-', {exclude: ['stopEarly', '--']});
	...
	parserOptions = minimist_options(parserOptions);
	...
	const argv = lib(options.argv, parserOptions);
	let help = redent(trimNewlines((options.help ?? '').replace(/\t+\n*$/, '')), 2);

	normalize(package_);

	external_node_process_namespaceObject.title = package_.bin ? Object.keys(package_.bin)[0] : package_.name;
	...
	const showHelp = code => {
		console.log(help);
		external_node_process_namespaceObject.exit(typeof code === 'number' ? code : 2);
	};
	...
	if (!options.allowUnknownFlags) {
		...
		if (unknownFlags.length > 0) {
			...
			external_node_process_namespaceObject.exit(2);
		}
	}
...
/* harmony default export */ const node_modules_meow = (meow);

;// CONCATENATED MODULE: ./src/summary/index.ts
function s(num) {
    return num === 1 ? "" : "s";
}
...
function yearReview(books, year) {
    ...
    const groupByMonth = groupBy(booksThisYear.map((b) => {
...
;// CONCATENATED MODULE: ./src/summary/cli.ts



const cli = node_modules_meow(`
	Usage
	  $ read-action-summary
...
const books = JSON.parse((0,external_fs_namespaceObject.readFileSync)(cli.flags.readFile, "utf-8"));
